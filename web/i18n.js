...
<li><a href="https://www.maptoolkit.org/" target="_blank" rel="noopener"><strong>Maptoolkit</strong></a>, per le mappe di base e i relativi stili cartografici.</li>
    ...
    rules: {
      ...
      p5: "Ciclabilità non consentita: è un percorso pedonale (esclusi gli attraversamenti) su cui non è espressamente indicato l'uso in bicicletta.",
      ...
      s10: "LTS impostato a 2: è un percorso o sentiero su cui è indicato di scendere dalla bicicletta.",
      s11: "LTS portato a 2: è un percorso condiviso con i pedoni (non segregato) in un punto dove convergono molte strade, con un livello di traffico misto pedoni/bici più stressante nelle ore di punta.",
      ...
      m18: "LTS impostato a 1: è una zona residenziale (living_street), con limite di velocità molto basso e priorità a pedoni e ciclisti per definizione.",
...
<p>Le statistiche sono <strong>sperimentali</strong> e derivano dallo stesso calcolo LTS descritto sulla pagina principale del progetto, basato sulle caratteristiche stradali di OpenStreetMap. Per i dettagli su affidabilità, metodo di raggruppamento e frequenza di aggiornamento, consulta le <a href="#" class="open-faq-link">FAQ</a> di questa pagina.</p>`,
    ...
    statsInterventionsTitle: "Strade dove potenzialmente sono necessari interventi",
    ...
    statsPriorityStreetsNote: "Strade il cui calcolo dell'LTS e la relazione con le altre strade della rete rendono il tratto potenzialmente importante da valutare per un intervento: tratti ad alto stress (LTS 3-4) che non hanno una pista ciclabile parallela nelle vicinanze, ordinati per gravità (classe LTS, poi centralità nella rete, poi lunghezza). Le vie senza nome sul catasto stradale non compaiono in elenco ma restano visibili sulla mappa.",
    ...
    statsClusterThresholdRangeTemplate: (popMin, popMax, areaMin, areaMax) => `Comuni con ${popMin}-${popMax} abitanti e superficie ${areaMin}-${areaMax} km²`,
    statsClusterThresholdMinTemplate: (popMin, areaMin, areaMax) => `Comuni con almeno ${popMin} abitanti e superficie ${areaMin}-${areaMax} km²`,
...
<li><a href="https://www.maptoolkit.org/" target="_blank" rel="noopener"><strong>Maptoolkit</strong></a>, for the base maps and their cartographic styles.</li>
    ...
    rules: {
      ...
      p5: "Cycling not permitted: it's a footway (excluding crossings) with no explicit indication that cycling is allowed.",
      ...
      s10: "Setting LTS to 2: it's a path or trail where you're required to dismount and walk the bike.",
      s11: "Increasing LTS to 2: it's a path shared with pedestrians (not segregated) at a point where many streets converge, making mixed foot/bike traffic more stressful at peak times.",
      ...
      m18: "Setting LTS to 1: it's a living street (\"zona residenziale\"), with a very low speed limit and pedestrian/cyclist priority by definition.",
...
<p>These statistics are <strong>experimental</strong> and come from the same LTS calculation described on the project's main page, based on OpenStreetMap's road characteristics. For details on reliability, the grouping method and how often data is updated, see this page's own <a href="#" class="open-faq-link">FAQ</a>.</p>`,
    ...
    statsInterventionsTitle: "Streets where intervention may be needed",
    ...
    statsPriorityStreetsNote: "Streets whose LTS score and relationship with the rest of the network make this stretch potentially worth evaluating for an intervention: high-stress stretches (LTS 3-4) with no separated cycle path nearby, ranked by severity (LTS class, then network centrality, then length). Unnamed streets don't appear in this list but stay visible on the map.",
    ...
    statsClusterThresholdRangeTemplate: (popMin, popMax, areaMin, areaMax) => `Municipalities with ${popMin}-${popMax} residents and ${areaMin}-${areaMax} km² of surface area`,
    statsClusterThresholdMinTemplate: (popMin, areaMin, areaMax) => `Municipalities with at least ${popMin} residents and ${areaMin}-${areaMax} km² of surface area`,
...
<li><a href="https://www.maptoolkit.org/" target="_blank" rel="noopener"><strong>Maptoolkit</strong></a>, für die Basiskarten und die zugehörigen kartografischen Stile.</li>
    ...
    rules: {
      ...
      p5: "Radfahren nicht erlaubt: Es handelt sich um einen Gehweg (ohne Querungen) ohne ausdrückliche Freigabe für Radfahrer.",
      ...
      s10: "LTS auf 2 gesetzt: Es handelt sich um einen Weg oder Pfad, auf dem das Fahrrad geschoben werden muss.",
      s11: "LTS auf 2 erhöht: Es handelt sich um einen mit Fußgängern geteilten (nicht getrennten) Weg an einem Punkt, an dem viele Straßen zusammenlaufen, wodurch der gemischte Fuß-/Radverkehr zu Stoßzeiten stressiger ist.",
      ...
      m18: "LTS auf 1 gesetzt: Es handelt sich um eine Verkehrsberuhigte Zone (living_street) mit sehr niedriger Höchstgeschwindigkeit und Vorrang für Fußgänger und Radfahrer per Definition.",
...
<p>Diese Statistiken sind <strong>experimentell</strong> und stammen aus derselben LTS-Berechnung, die auf der Hauptseite des Projekts beschrieben wird, basierend auf den Straßenmerkmalen von OpenStreetMap. Details zu Zuverlässigkeit, Gruppierungsmethode und Aktualisierungshäufigkeit finden Sie in den <a href="#" class="open-faq-link">FAQ</a> dieser Seite.</p>`,
    ...
    statsInterventionsTitle: "Straßen, an denen möglicherweise Maßnahmen nötig sind",
    ...
    statsPriorityStreetsNote: "Straßen, bei denen die LTS-Berechnung und die Beziehung zum übrigen Netz den Abschnitt als potenziell bewertenswert für eine Maßnahme erscheinen lassen: Abschnitte mit hohem Stress (LTS 3-4) ohne parallelen Radweg in der Nähe, sortiert nach Schweregrad (LTS-Klasse, dann Netz-Zentralität, dann Länge). Namenlose Straßen erscheinen nicht in dieser Liste, bleiben aber auf der Karte sichtbar.",
    ...
    statsClusterThresholdRangeTemplate: (popMin, popMax, areaMin, areaMax) => `Gemeinden mit ${popMin}-${popMax} Einwohnern und ${areaMin}-${areaMax} km² Fläche`,
    statsClusterThresholdMinTemplate: (popMin, areaMin, areaMax) => `Gemeinden mit mindestens ${popMin} Einwohnern und ${areaMin}-${areaMax} km² Fläche`,
...
<li><a href="https://www.maptoolkit.org/" target="_blank" rel="noopener"><strong>Maptoolkit</strong></a>, pour les fonds de carte et les styles cartographiques associés.</li>
    ...
    rules: {
      ...
      p5: "Vélo non autorisé : il s'agit d'un chemin piéton (hors traversées) sans autorisation explicite pour les vélos.",
      ...
      s10: "LTS fixé à 2 : il s'agit d'un chemin ou sentier où il est indiqué de mettre pied à terre.",
      s11: "LTS porté à 2 : il s'agit d'une voie partagée avec les piétons (non séparée) à un point où convergent de nombreuses rues, ce qui rend la circulation mixte piétons/vélos plus stressante aux heures de pointe.",
      ...
      m18: "LTS fixé à 1 : il s'agit d'une zone de rencontre (living_street), avec une vitesse maximale très basse et une priorité aux piétons/cyclistes par définition.",
...
<p>Ces statistiques sont <strong>expérimentales</strong> et proviennent du même calcul LTS décrit sur la page principale du projet, basé sur les caractéristiques routières d'OpenStreetMap. Pour les détails sur la fiabilité, la méthode de regroupement et la fréquence de mise à jour, consultez la <a href="#" class="open-faq-link">FAQ</a> de cette page.</p>`,
    ...
    statsInterventionsTitle: "Rues où une intervention pourrait être nécessaire",
    ...
    statsPriorityStreetsNote: "Rues dont le calcul du LTS et la relation avec le reste du réseau rendent le tronçon potentiellement important à évaluer pour une intervention : tronçons à stress élevé (LTS 3-4) sans piste cyclable séparée à proximité, classés par gravité (classe LTS, puis centralité dans le réseau, puis longueur). Les rues sans nom n'apparaissent pas dans cette liste mais restent visibles sur la carte.",
    ...
    statsClusterThresholdRangeTemplate: (popMin, popMax, areaMin, areaMax) => `Communes avec ${popMin}-${popMax} habitants et une superficie de ${areaMin}-${areaMax} km²`,
    statsClusterThresholdMinTemplate: (popMin, areaMin, areaMax) => `Communes avec au moins ${popMin} habitants et une superficie de ${areaMin}-${areaMax} km²`,
