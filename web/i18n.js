...
<p>Il tempo stimato ipotizza una <strong>bicicletta muscolare</strong> (non elettrica), con una velocità di base di 18 km/h in piano, ridotta in salita e aumentata (fino a un limite) in discesa, in base alla pendenza reale rilevata dal modello del terreno lungo il percorso.</p>
...
<p>Per tutto il resto - <strong>l'algoritmo, il calcolo o il software</strong> - il codice pubblico permette di verificare il funzionamento e contribuire al suo miglioramento.</p>`,
  ...
  en: {
    ...
    routingDisclaimerBody: "The route favours the lowest-stress (LTS) stretches available and excludes segments classified as not suitable for cycling: if the only connection to a destination runs through one, a route may not be found. It doesn't account for real-time traffic, since that data isn't available. This is an experimental engine that computes the route locally, on your own device. The estimated time assumes a muscular (non-electric) bike and accounts for the terrain's real gradient, but not wind, fitness level, or stops.",
    routeEstimatedTimeTemplate: (h, m) => (h > 0 ? `Estimated time: ~${h} h ${m} min` : `Estimated time: ~${m} min`),
...
<p>Und vor allem, <strong>wo man besser Rad fahren könnte</strong>.</p>`,
  ...
  fr: {
    ...
    routingDisclaimerBody: "L'itinéraire privilégie les tronçons avec le niveau de stress (LTS) le plus bas et exclut les tronçons classés comme non adaptés au vélo : si la seule liaison vers une destination passe par là, il se peut qu'aucun itinéraire ne soit trouvé. Il ne tient pas compte du trafic en temps réel, cette donnée n'étant pas disponible. Il s'agit d'un moteur expérimental qui calcule l'itinéraire localement, sur votre propre appareil. Le temps estimé suppose un vélo musculaire (non électrique) et tient compte de la pente réelle du terrain, mais pas du vent, de la forme physique ni des arrêts.",
    routeEstimatedTimeTemplate: (h, m) => (h > 0 ? `Temps estimé : ~${h} h ${m} min` : `Temps estimé : ~${m} min`),
