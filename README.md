...
- LTS classification engine for edges and nodes with explicit decision-rule mapping (including `highway=steps`: not bikeable - LTS 0 - unless the way carries a bicycle ramp, `ramp=yes` or `ramp:bicycle=yes`, which makes it LTS 1).
...
A site-level `<header>` sits above the map (title + current area, language switcher, and nav buttons) - separate from the map-level controls panel (legend, basemap, terrain, gap toggle) that floats over the map itself. The **"About"** nav button opens a panel with a short project blurb and the citation for the paper the tool is based on (Venturoso et al., 2026 - same reference as the Citation section above), with a link to the DOI. The **"Privacy"** nav button opens a placeholder panel - the site sets no cookies or tracking today, so there's nothing to consent to yet, but the panel is there to be filled in if that changes. Both are same-page modals (`.info-panel` in `web/index.html`), not separate pages, wired through a small reusable `setupInfoPanel()` helper so a third nav link reuses the same pattern.
